...
class Myclass extends CController {
    ...
    public static function rememberMeAdmin($email, $check) {
        if ($check > 0) {
            $time = time();     // Gets the current server time
            $cookie = new CHttpCookie('admin_email', $email);

            $cookie->expire = $time + 60 * 60 * 24 * 30;               // 30 days
            Yii::app()->request->cookies['admin_email'] = $cookie;
        } else {
            unset(Yii::app()->request->cookies['admin_email']);
        }
    }
